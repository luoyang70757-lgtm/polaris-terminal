...
function verifyByReadback(sftp, remotePath, localPath, total) {
  ...
  return new Promise((resolve) => {
    ...
    rs.on('end', () => {
      if (got !== total) return resolve({ ok: false, got });
      let local;
      try { local = fs.readFileSync(localPath); } catch { return resolve({ ok: false, got: -1 }); }
      const remote = Buffer.concat(chunks, got);
      resolve({ ok: remote.equals(local), got });
    });
  });
}
...
function uploadFile(sftp, localPath, remotePath, onProgress, resumeFrom) {
  ...
  return runOnce(resumeFrom).then(verify).then((badSize) => {
    ...
    return runOnce(0).then(verify).then((badSize2) => {
      ...
      // stat 两次都不符(含远端 0B)→ 读回远端核对:区分"设备 stat 上报坏(数据其实在)"和"真没传上"。
      // 某些设备(如 H3C 网络设备)SFTP 对已写入文件 stat 恒报 0,但内容已落盘 —— 不能据此删文件。
      return verifyByReadback(sftp, remotePath, localPath, total).then((rb) => {
        if (rb.ok) return; // 读回一致 → 数据已落盘,设备 stat 骗人,视为成功
        return new Promise((res) => sftp.unlink(remotePath, () => res())).then(() => {
          throw new Error(`上传校验失败:远端 ${badSize2}B ≠ 本地 ${total}B(读回仅 ${rb.got}B,已删除远端残缺文件,请重试)`);
        });
      });
    });
  });
