...
// 传输假死判定:超过该时长零字节推进(任何真实传输都在持续推字节)= 连接已死
// (掉线无 RST/设备卡住)。超时主动终止,而非让 rs.pipe(ws) 永久等待 → 进度停在原地的"卡住不动"。
const TRANSFER_STALL_MS = 120 * 1000; // 120s

// 用户取消的标志性错误:调用方据此区分「取消」与「真失败」——
//   取消:保留本地/远端半成品 + 记下续传点(下次同路径从断点续),绝不删残缺文件;
//   真失败(校验不符/连接坏):按原逻辑删残缺 + 报错。
// (旧版取消只 destroy 流、不结算 promise → ws 'close' 把 promise resolve 成"成功" →
//  走校验分支把半成品 unlink 掉,且因文件已删而记不下续传点 —— 见 docs/issues-2026-09-11.md 第 2 条)
...
// 30s 超时兜底:慢/串行设备(H3C)在通道忙时 stat 可能挂住 → 超时返回 null(不可对账),不阻塞流程。
...
function uploadFile(sftp, localPath, remotePath, onProgress, resumeFrom, shouldCancel) {
  const fs = require('fs');
  ...
  const runOnce = (from) => new Promise((resolve, reject) => {
    ...
    // 心跳 watchdog:每 200ms 查「取消」和「长时间无字节推进」。
    // 连接假死(掉线无 RST/设备卡住)时 rs.pipe(ws) 会永久等待 → 进度停在原地,
    // 就是用户看到的"大文件上传卡住不动"。超过 TRANSFER_STALL_MS 零数据 → 主动终止报错。
    const watchdog = setInterval(() => {
      ...
      if (shouldCancel && shouldCancel()) {
        ...
        // 取消:远端已写部分**保留**(供续传),不走校验/删除分支;由调用方记中断点 + 标「已取消」
        reject(cancelError());
        return;
      }
      if (Date.now() - lastData > TRANSFER_STALL_MS) {
        cancelled = true;
        doneOnce();
        try { ws.destroy(); } catch { /* ignore */ }
        try { rs.destroy(); } catch { /* ignore */ }
        // 假死超时同理:半成品保留(调用方会记中断点),不删远端——否则"断点续传"永远无从续起
        reject(new Error(`传输超时(${Math.round(TRANSFER_STALL_MS / 1000)}s 无数据,连接可能已断开)`));
      }
    }, 200);
    const doneOnce = () => clearInterval(watchdog);
    // 同下载:任一出错成对销毁(否则本地读句柄/远端写句柄泄漏)
    const abortUp = (e) => {
      doneOnce();
      try { rs.destroy(); } catch { /* ignore */ }
      try { ws.destroy(); } catch { /* ignore */ }
      reject(new Error(`${localPath} → ${remotePath}: ${e.message}`));
    };
    rs.on('error', abortUp);
    ws.on('error', abortUp);
    rs.on('data', (c) => { lastData = Date.now(); done += c.length; if (onProgress) onProgress(done, total); });
    rs.pipe(ws);
    ws.on('close', () => { doneOnce(); resolve(); });
    ws.on('finish', () => { doneOnce(); resolve(); });
  });
  // 校验:stat 撒谎报 0 或读回不可靠(H3C 返回 0/报错)都按成功 —— 写入流完整结束是数据落盘的可靠信号;
  // 只有读回成功且读出**非零部分**(0<got<total,正常服务器真截断)才判失败删文件。
  ...
  const readbackOrFail = () => verifyByReadback(sftp, remotePath, localPath, total).then((rb) => {
    if (rb.ok) return;
    // H3C 等设备读回不可靠:读回流报错(got=-1)或返回 0(got=0,设备 read 也撒谎)
    // 都不能证明上传失败 → 信任写入流,不删文件
    if (rb.got === -1 || rb.got === 0) return;
    // 只有读回成功且读出非零部分(真截断,正常服务器)才删残缺 + 明确报错
  ...
  return runOnce(resumeFrom).then(verify).then((badSize) => {
    ...
    // 此时 stat=0 是设备撒谎,不是失败 → 信任成功,不再读回
    if (badSize === 0) return;
    // stat 报非 0 但≠本地:重传一次,仍不符再读回核对(读回 0/报错按成功,非零部分按真截断)
...
function downloadFile(sftp, remotePath, localPath, onProgress, resumeFrom, shouldCancel) {
  const fs = require('fs');
  return statSize(sftp, remotePath).then((total) => new Promise((resolve, reject) => {
    ...
    const settle = () => {
      if (settled) return;
      settled = true;
      if (total === null) return resolve(); // stat 超时/不可对账:流完整结束即成功,不做大小校验
      const got = fs.statSync(localPath).size;
      // 远端报 0 字节:无法区分"真空文件"与"设备撒谎"(H3C 类设备对文件恒报 0 且读回也为空)。
      // 这里留一条痕迹,便于从日志判断"下下来是空文件"的原因(不改变成败判定:0==0 视为成功)。
      if (total === 0 && got === 0) {
        try { console.warn(`[SFTP] 下载得到 0 字节文件(远端也报 0,可能是设备撒谎): ${remotePath}`); } catch { /* ignore */ }
        return resolve();
      }
    ...
    let lastData = Date.now();
    // 心跳 watchdog:查取消 + 查长时间无字节推进(连接假死 → 主动终止,否则进度停在原地的"卡住不动")。
    const watchdog = setInterval(() => {
      if (cancelled) return;
      if (shouldCancel && shouldCancel()) {
        cancelled = true;
        settled = true; // 阻止随后 ws 'close' 触发的 settle():它会按大小不符把本地半成品 unlink
        doneOnce();
        try { rs.destroy(); } catch { /* ignore */ }
        try { ws.destroy(); } catch { /* ignore */ }
        reject(cancelError()); // 本地已落盘部分保留 → 下次同路径续传
        return;
      }
      if (Date.now() - lastData > TRANSFER_STALL_MS) {
        cancelled = true;
        settled = true; // 同上:假死超时也保住半成品,否则下载永远无法续传
        doneOnce();
        try { rs.destroy(); } catch { /* ignore */ }
        try { ws.destroy(); } catch { /* ignore */ }
        reject(new Error(`传输超时(${Math.round(TRANSFER_STALL_MS / 1000)}s 无数据,连接可能已断开)`));
    ...
    const doneOnce = () => clearInterval(watchdog);
    // 读/写任一出错都要**成对销毁**两个流:只 reject 不销毁会让对端流的 fd 一直开着
    // (实测:远端读失败后本地写句柄泄漏,进程 lsof 里能看到 0 字节文件被长期持有)
    const abort = (e) => {
      doneOnce();
      try { rs.destroy(); } catch { /* ignore */ }
      try { ws.destroy(); } catch { /* ignore */ }
      reject(new Error(`${remotePath} → ${localPath}: ${e.message}`));
    };
    rs.on('error', abort);
    ws.on('error', abort);
    rs.on('data', (c) => { lastData = Date.now(); done += c.length; if (onProgress) onProgress(done, total); });
    rs.pipe(ws);
    ws.on('close', () => { doneOnce(); settle(); });
    ws.on('finish', () => { doneOnce(); settle(); });
  }));
}
...
async function uploadDir(sftp, localDir, remoteDir, onProgress, resumeFromOf, onFileError, shouldCancel) {
  const fs = require('fs');
  const path = require('path');
  ...
  // 取消时不再启动新文件(mapConcurrent 的 worker 检查 shouldCancel)
  await mapConcurrent(files, SFTP_CONCURRENCY, async (f, i) => {
    if (shouldCancel && shouldCancel()) return;
    let last = 0;
    try {
      const rFrom = await resumeFromOf(f.lp, f.rp); // 该文件若有中断点则从那里续,否则全量
      await uploadFile(sftp, f.lp, f.rp, (d) => { done += (d - last); last = d; if (onProgress) onProgress({ done, total, file: f.rp, fileDone: d, fileTotal: f.size, filesDone: finished, filesTotal: files.length }); }, rFrom, shouldCancel);
...
async function downloadDir(sftp, remoteDir, localDir, onProgress, shouldCancel) {
  const fs = require('fs');
  const path = require('path');
  ...
  await mapConcurrent(files, SFTP_CONCURRENCY, async (f, i) => {
    ...
    let last = 0;
    await downloadFile(sftp, f.rp, f.lp, (d) => { done += (d - last); last = d; if (onProgress) onProgress({ done, total, file: f.rp, fileDone: d, fileTotal: f.size, filesDone: finished, filesTotal: files.length }); }, 0, shouldCancel);
    locals[i] = f.lp;
    finished++;
    if (onProgress) onProgress({ done, total, file: f.rp, fileDone: f.size, fileTotal: f.size, filesDone: finished, filesTotal: files.length });
  });
