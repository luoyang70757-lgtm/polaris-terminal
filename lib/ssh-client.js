...
function uploadFile(sftp, localPath, remotePath, onProgress, resumeFrom, shouldCancel) {
  ...
  const runOnce = (from) => new Promise((resolve, reject) => {
    ...
    rs.on('error', (e) => { doneOnce(); reject(new Error(`${localPath} → ${remotePath}: ${e.message}`)); });
    ws.on('error', (e) => { doneOnce(); reject(new Error(`${localPath} → ${remotePath}: ${e.message}`)); });
...
function downloadFile(sftp, remotePath, localPath, onProgress, resumeFrom, shouldCancel) {
  ...
  return statSize(sftp, remotePath).then((total) => new Promise((resolve, reject) => {
    ...
    const doneOnce = () => clearInterval(watchdog);
    rs.on('error', (e) => { doneOnce(); reject(new Error(`${remotePath} → ${localPath}: ${e.message}`)); });
    ws.on('error', (e) => { doneOnce(); reject(new Error(`${remotePath} → ${localPath}: ${e.message}`)); });
    rs.on('data', (c) => { lastData = Date.now(); done += c.length; if (onProgress) onProgress(done, total); });
    rs.pipe(ws);
