...
function createStore(initialBytes) {
  ...
  // ---- 堡垒机资产缓存表(2026-08-14 新增)----
  // 捕获到的 H3C/JumpServer 资产列表持久化到这里(随整库 AES 加密落盘),重启不丢。
  // 资产不含密码(连接走堡垒机一次性凭证),所以可以直接明文入库;按堡垒机地址区分多台堡垒机。
  // 目录请求是渐进式补齐的,dir 字段可能为空(未分组),favorite 标记收藏。
  db.exec(`
    CREATE TABLE IF NOT EXISTS bastion_assets (
      bastion_url    TEXT NOT NULL,             -- 堡垒机 Web 地址(如 https://10.204.240.4)
      dev_id         TEXT NOT NULL,             -- 设备 ID(堡垒机侧唯一)
      name           TEXT NOT NULL,             -- 设备名
      ip             TEXT DEFAULT '',           -- IP
      port           INTEGER DEFAULT 22,        -- SSH 端口
      proto          TEXT DEFAULT 'ssh',        -- 协议
      accounts       TEXT DEFAULT '[]',         -- 可登录账号 JSON 数组
      recent_account TEXT DEFAULT '',           -- 最近使用的账号
      dir            TEXT DEFAULT '',           -- 主业务目录名(最后一个匹配的目录)
      dirs           TEXT DEFAULT '[]',         -- 所属全部业务目录名 JSON 数组(设备可属多个目录,网页每个都显示)
      dir_path       TEXT DEFAULT '',           -- 主目录 path JSON 数组
      favorite       INTEGER DEFAULT 0,         -- 1=收藏设备
      source         TEXT DEFAULT 'h3c',        -- 来源: h3c(webview 捕获) | jms(REST API)
      fav_group      TEXT DEFAULT '',           -- 手动收藏分组路径(如 "组1/组2";空=未加入手动收藏分组)
      updated_at     TEXT DEFAULT (datetime('now', 'localtime')),
      PRIMARY KEY (bastion_url, dev_id)
    )
  `);
  // 旧库迁移:补 dirs/source/fav_group 列(SQLite 的 ADD COLUMN 无 IF NOT EXISTS,先查再加)
  try {
    const cols = db.prepare('PRAGMA table_info(bastion_assets)').all();
    const has = (n) => cols.some((c) => c.name === n);
    if (!has('dirs')) db.exec(`ALTER TABLE bastion_assets ADD COLUMN dirs TEXT DEFAULT '[]'`);
    if (!has('source')) db.exec(`ALTER TABLE bastion_assets ADD COLUMN source TEXT DEFAULT 'h3c'`);
    if (!has('fav_group')) db.exec(`ALTER TABLE bastion_assets ADD COLUMN fav_group TEXT DEFAULT ''`);
  } catch { /* 迁移失败不影响使用,新增列走默认 */ }
  const stmtBastionUpsert = db.prepare(`
    INSERT INTO bastion_assets
      (bastion_url, dev_id, name, ip, port, proto, accounts, recent_account, dir, dirs, dir_path, favorite, source, fav_group, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now', 'localtime'))
    ON CONFLICT(bastion_url, dev_id) DO UPDATE SET
      name=excluded.name, ip=excluded.ip, port=excluded.port, proto=excluded.proto,
      accounts=excluded.accounts, recent_account=excluded.recent_account,
      dir=excluded.dir, dirs=excluded.dirs, dir_path=excluded.dir_path, favorite=excluded.favorite,
      source=excluded.source, fav_group=excluded.fav_group,
      updated_at=datetime('now', 'localtime')
  `);
  const stmtBastionClear = db.prepare('DELETE FROM bastion_assets WHERE bastion_url = ? AND source = ?');
  const stmtBastionClearAll = db.prepare('DELETE FROM bastion_assets');
  const stmtFavGroupClear = db.prepare(`UPDATE bastion_assets SET fav_group = '' WHERE bastion_url = ? AND (fav_group = ? OR fav_group LIKE ?)`);

  // ---- 手动收藏分组表(堡垒机收藏:用户自己建组、把主机放进去,不再依赖 H3C webview 收藏夹) ----
  // bastion_key = 堡垒机连接 origin(如 https://10.204.240.4 / http://192.168.1.250),收藏分组按连接隔离
  db.exec(`
    CREATE TABLE IF NOT EXISTS bastion_fav_groups (
      id          INTEGER PRIMARY KEY AUTOINCREMENT,
      bastion_key TEXT DEFAULT '',
      name        TEXT NOT NULL,
      parent_id   INTEGER DEFAULT NULL,
      created_at  TEXT DEFAULT (datetime('now', 'localtime')),
      UNIQUE(bastion_key, name)
    )
  `);
  // 旧库迁移:早期版本 name UNIQUE、无 bastion_key 列 → 重建为 (bastion_key, name) 唯一
  try {
    const fgCols = db.prepare('PRAGMA table_info(bastion_fav_groups)').all();
    if (!fgCols.some((c) => c.name === 'bastion_key')) {
      db.exec(`
        ALTER TABLE bastion_fav_groups RENAME TO bastion_fav_groups_old;
        CREATE TABLE bastion_fav_groups (
          id INTEGER PRIMARY KEY AUTOINCREMENT, bastion_key TEXT DEFAULT '',
          name TEXT NOT NULL, parent_id INTEGER DEFAULT NULL,
          created_at TEXT DEFAULT (datetime('now','localtime')), UNIQUE(bastion_key, name)
        );
        INSERT INTO bastion_fav_groups (id, bastion_key, name, parent_id, created_at)
          SELECT id, '', name, parent_id, created_at FROM bastion_fav_groups_old;
        DROP TABLE bastion_fav_groups_old;
      `);
    }
  } catch { /* 迁移失败不影响使用 */ }
  const stmtFavGroupInsert = db.prepare('INSERT INTO bastion_fav_groups (bastion_key, name, parent_id) VALUES (?, ?, ?)');
  const stmtFavGroupRename = db.prepare('UPDATE bastion_fav_groups SET name = ? WHERE id = ?');
  const stmtFavGroupDelete = db.prepare('DELETE FROM bastion_fav_groups WHERE id = ?');
  const stmtFavGroupGet = db.prepare('SELECT id, name, parent_id, bastion_key FROM bastion_fav_groups WHERE id = ?');
  const stmtListFavGroups = db.prepare('SELECT id, bastion_key, name, parent_id FROM bastion_fav_groups ORDER BY bastion_key, name');
  const stmtFavGroupChildren = db.prepare('SELECT id, name, parent_id, bastion_key FROM bastion_fav_groups WHERE parent_id = ?');
  ...
  return {
    ...
    deleteGroup(id) {
      ...
      // 两个 DELETE 包事务:中途失败整体回滚,不留"分组删了会话还在/会话没了分组还在"的残库
      db.exec('BEGIN');
      try {
    ...
    /** 列出某台主机的命令记录(智能命令推荐用;主机为空则返回全部) */
    listCmdsByHost(host) {
      if (!host) return this.listCmds();
      return db.prepare('SELECT host, command, created_at FROM cmd_history WHERE host = ? ORDER BY id').all(String(host));
    },
    ...
    /** 批量保存某台堡垒机的资产(整批覆盖:先清空该堡垒机旧数据再插入,避免残留已下线的设备)
     *  source: 'h3c'(webview 捕获) | 'jms'(REST API);JMS 资产的 bastion_url 用服务器 origin */
    saveBastionAssets(bastionUrl, assets, source) {
      const url = String(bastionUrl || '').trim();
      if (!url) return 0;
      const src = source === 'jms' ? 'jms' : 'h3c';
      const list = Array.isArray(assets) ? assets : [];
      db.exec('BEGIN');
      try {
        stmtBastionClear.run(url, src);
        for (const a of list) {
          stmtBastionUpsert.run(
            url,
            String(a.devId != null ? a.devId : a.id || ''),
            String(a.name || ''),
            String(a.ip || a.address || ''),
            Number(a.port) || 22,
            String(a.proto || 'ssh'),
            JSON.stringify(Array.isArray(a.accounts) ? a.accounts : []),
            String(a.recentAccount || ''),
            String(a.dir || ''),
            JSON.stringify(Array.isArray(a.dirs) ? a.dirs : (a.dir ? [a.dir] : [])),
            JSON.stringify(Array.isArray(a.dirPath) ? a.dirPath : []),
            a.favorite ? 1 : 0,
            src,
            String(a.favGroup || a.fav_group || '')
          );
        }
        db.exec('COMMIT');
      } catch (err) {
    ...
    /** 读出全部堡垒机资产(按堡垒机地址分组,dir/accounts 还原成对象) */
    loadBastionAssets() {
      const rows = db.prepare('SELECT * FROM bastion_assets ORDER BY bastion_url, name').all();
      const byUrl = {};
      for (const r of rows) {
        let accounts = [];
        try { accounts = JSON.parse(r.accounts || '[]'); } catch { /* 损坏则空 */ }
        let dirPath = [];
        try { dirPath = JSON.parse(r.dir_path || '[]'); } catch { /* 损坏则空 */ }
        let dirs = [];
        try { dirs = JSON.parse(r.dirs || '[]'); } catch { /* 损坏则空 */ }
        const asset = {
          devId: r.dev_id, name: r.name, ip: r.ip, port: r.port, proto: r.proto,
          accounts, recentAccount: r.recent_account, dir: r.dir, dirs,
          dirPath, favorite: !!r.favorite, source: r.source || 'h3c',
          favGroup: r.fav_group || '', updatedAt: r.updated_at,
        };
        (byUrl[r.bastion_url] = byUrl[r.bastion_url] || []).push(asset);
      }
      return byUrl;
    },

    /** 列出全部手动收藏分组(堡垒机收藏;用户自建,非 webview);每项含 bastion_key(连接 origin) */
    listBastionFavGroups() {
      return stmtListFavGroups.all();
    },

    /** 新建手动收藏分组,返回新分组 id;parentId 传了就建为它的子分组;bastionKey=连接 origin,按连接隔离 */
    createBastionFavGroup(name, parentId, bastionKey) {
      if (!name || !name.trim()) throw new Error('收藏分组名不能为空');
      const r = stmtFavGroupInsert.run(String(bastionKey || ''), name.trim(), parentId || null);
      return Number(r.lastInsertRowid);
    },

    /** 重命名手动收藏分组 */
    renameBastionFavGroup(id, name) {
      if (!name || !name.trim()) throw new Error('收藏分组名不能为空');
      stmtFavGroupRename.run(name.trim(), id);
    },

    /** 删除手动收藏分组(递归后代),并清掉**该连接**资产上指向该组(或其子组)的 fav_group */
    deleteBastionFavGroup(id) {
      // 收集该组全部后代 id
      const ids = [Number(id)];
      let cursor = [Number(id)];
      while (cursor.length) {
        const next = [];
        for (const gid of cursor) {
          for (const child of stmtFavGroupChildren.all(gid)) next.push(child.id);
        }
        ids.push(...next); cursor = next;
      }
      const marks = ids.map(() => '?').join(',');
      db.exec('BEGIN');
      try {
        // 清掉指向这些组路径的资产 fav_group("组A" 与 "组A/子组"),且只清同一 bastion_key 的连接
        for (const gid of ids) {
          const g = stmtFavGroupGet.get(gid);
          if (g) stmtFavGroupClear.run(g.bastion_key || '', g.name, g.name + '/%');
        }
        db.prepare(`DELETE FROM bastion_fav_groups WHERE id IN (${marks})`).run(...ids);
        db.exec('COMMIT');
      } catch (err) {
        db.exec('ROLLBACK');
        throw err;
    ...
    /** 删除某台堡垒机的全部资产(用户删除堡垒机配置时调用);两种来源都清 */
    deleteBastionAssets(bastionUrl) {
      const url = String(bastionUrl || '');
      if (!url) return;
      const stmt = db.prepare('DELETE FROM bastion_assets WHERE bastion_url = ?');
      stmt.run(url);
    },

    /** 清空全部堡垒机资产(清除历史时调用) */
    deleteAllBastionAssets() {
      stmtBastionClearAll.run();
    },
