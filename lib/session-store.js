...
function createStore(initialBytes) {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS sessions (
      ...
      username    TEXT NOT NULL,                      -- 登录用户(必填)
  ...
  const stmtInsert = db.prepare(`
    INSERT INTO sessions (name, host, port, username, password, private_key, passphrase, encoding, group_id, tag_color, on_connect, jump)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const stmtUpdate = db.prepare(`
    UPDATE sessions
    SET name = ?, host = ?, port = ?, username = ?, password = ?,
        private_key = ?, passphrase = ?, encoding = ?, group_id = ?, tag_color = ?,
  ...
  return {
    ...
    create(s) {
      const r = stmtInsert.run(s.name, s.host, s.port || 22, s.username, s.password || '',
        s.privateKey || '', s.passphrase || '', s.encoding || 'utf8', resolveGroupId(s), s.tagColor || '', s.onConnect || '', s.jump || '');
    ...
    update(id, s) {
      stmtUpdate.run(s.name, s.host, s.port || 22, s.username, s.password || '',
        s.privateKey || '', s.passphrase || '', s.encoding || 'utf8', resolveGroupId(s), s.tagColor || '', s.onConnect || '', s.jump || '', id);
    },
        ...
        for (const item of list) {
          try {
            const r = stmtInsert.run(
              ...
              item.encoding || 'utf8', resolveGroupId(item)
            );
