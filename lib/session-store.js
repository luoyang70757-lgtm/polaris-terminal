...
function createStore(initialBytes) {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS bastion_assets (
      ...
      dir            TEXT DEFAULT '',           -- 业务目录名(按目录请求补充分组)
      dir_path       TEXT DEFAULT '',           -- 目录 path JSON 数组
  ...
  const stmtBastionUpsert = db.prepare(`
    INSERT INTO bastion_assets
      (bastion_url, dev_id, name, ip, port, proto, accounts, recent_account, dir, dir_path, favorite, updated_at)
    ...
    ON CONFLICT(bastion_url, dev_id) DO UPDATE SET
      ...
      dir=excluded.dir, dir_path=excluded.dir_path, favorite=excluded.favorite,
    ...
    loadBastionAssets() {
      ...
      for (const r of rows) {
        ...
        const asset = {
          ...
          accounts, recentAccount: r.recent_account, dir: r.dir, dirPath,
          favorite: !!r.favorite, updatedAt: r.updated_at,
        };
