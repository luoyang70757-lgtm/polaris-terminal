...
function createStore(initialBytes) {
  ...
  `);
  // 旧库迁移:补 dirs 列(SQLite 的 ADD COLUMN 无 IF NOT EXISTS,先查再加)
  try {
    const cols = db.prepare('PRAGMA table_info(bastion_assets)').all();
    if (!cols.some((c) => c.name === 'dirs')) db.exec(`ALTER TABLE bastion_assets ADD COLUMN dirs TEXT DEFAULT '[]'`);
  } catch { /* 迁移失败不影响使用,dirs 走空 */ }
  const stmtBastionUpsert = db.prepare(`
    INSERT INTO bastion_assets
      (bastion_url, dev_id, name, ip, port, proto, accounts, recent_account, dir, dirs, dir_path, favorite, updated_at)
  ...
  `);
  const stmtBastionClear = db.prepare('DELETE FROM bastion_assets WHERE bastion_url = ?');
  ...
  return {
    ...
    saveBastionAssets(bastionUrl, assets) {
      const url = String(bastionUrl || '').trim();
      if (!url) return 0;
      const list = Array.isArray(assets) ? assets : [];
      db.exec('BEGIN');
      try {
        stmtBastionClear.run(url);
        for (const a of list) {
    ...
    loadBastionAssets() {
      ...
      for (const r of rows) {
        ...
        const asset = {
          ...
          dirPath, favorite: !!r.favorite, updatedAt: r.updated_at,
        };
    ...
    /** 删除某台堡垒机的全部资产(用户删除堡垒机配置时调用) */
    deleteBastionAssets(bastionUrl) {
      stmtBastionClear.run(String(bastionUrl || ''));
    },
