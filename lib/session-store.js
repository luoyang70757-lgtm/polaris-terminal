...
function createStore(initialBytes) {
  ...
  const stmtFavGroupClear = db.prepare(`UPDATE bastion_assets SET fav_group = '' WHERE fav_group = ? OR fav_group LIKE ?`);
  ...
  `);
  const stmtFavGroupInsert = db.prepare('INSERT INTO bastion_fav_groups (name, parent_id) VALUES (?, ?)');
  ...
  const stmtFavGroupGet = db.prepare('SELECT id, name, parent_id FROM bastion_fav_groups WHERE id = ?');
  const stmtFavGroupGetByName = db.prepare('SELECT id FROM bastion_fav_groups WHERE name = ?');
  const stmtListFavGroups = db.prepare('SELECT id, name, parent_id FROM bastion_fav_groups ORDER BY name');
  const stmtFavGroupChildren = db.prepare('SELECT id, name, parent_id FROM bastion_fav_groups WHERE parent_id = ?');
  ...
  return {
    ...
    /** 列出全部手动收藏分组(堡垒机收藏;用户自建,非 webview) */
    listBastionFavGroups() {
      return stmtListFavGroups.all();
    },

    /** 新建手动收藏分组,返回新分组 id;parentId 传了就建为它的子分组 */
    createBastionFavGroup(name, parentId) {
      if (!name || !name.trim()) throw new Error('收藏分组名不能为空');
      const r = stmtFavGroupInsert.run(name.trim(), parentId || null);
      return Number(r.lastInsertRowid);
    },
    ...
    /** 删除手动收藏分组(递归后代),并清掉资产上指向该组(或其子组)的 fav_group */
    deleteBastionFavGroup(id) {
      ...
      db.exec('BEGIN');
      try {
        // 清掉指向这些组路径的资产 fav_group("组A" 与 "组A/子组")
        for (const gid of ids) {
          const g = stmtFavGroupGet.get(gid);
          if (g) stmtFavGroupClear.run(g.name, g.name + '/%');
        }
