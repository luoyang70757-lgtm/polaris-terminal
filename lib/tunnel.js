  ...
  return new Promise((resolve, reject) => {
    const server = net.createServer((socket) => {
      ...
      socket.on('data', (buf) => {
        try {
          if (phase === 0) {
            if (buf[0] !== 5) { socket.destroy(); return; }
            socket.write(Buffer.from([5, 0])); // 无需认证
            phase = 1;
            return;
          }
          const target = parseSocks5Request(buf);
          if (!target) { socket.write(SOCKS_FAIL); socket.destroy(); return; }
          conn.forwardOut('127.0.0.1', 0, target.host, target.port, (err, stream) => {
            ...
            // 必须一起转发,否则会丢包
            const leftover = buf.slice(target.headerLen);
