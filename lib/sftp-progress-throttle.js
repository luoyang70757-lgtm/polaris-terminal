  ...
  return {
    ...
    emit(payload) {
      ...
      if (st && now - st.lastTs < intervalMs) {
        // 文件切换(远程路径不同)→ 立即发:多文件/递归上传时保证面板每文件一行
        // (同一文件的进度仍节流,平滑且不刷屏)
        if (st.lastFile !== payload.file) {
          st.lastTs = now; st.lastFile = payload.file; st.pending = null;
          send(payload);
          return;
        }
      ...
      entries.set(payload.jobId, { lastTs: now, pending: null, lastFile: payload.file });
      send(payload);
    },
