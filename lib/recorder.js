...
/**
 * 录制结束:把 .jsonl 压缩成 .jsonl.gz,返回 { size, file }。
 * 录制过程仍按 JSONL 追加(写入热路径不变),停止这一刻才压缩收档 —— 磁盘上的录制即压缩态。
 * 压缩失败(罕见)保留原 .jsonl 返回,回放端对无 .gz 后缀的文件照常解析,不阻塞收尾。
 */
function finishRecording(filePath) {
  try {
    ...
    fs.rmSync(filePath);
    return { size: gz.length, file: gzPath };
  } catch {
    return { size: fs.statSync(filePath).size, file: filePath };
  }
}

/**
 * 读取并解析一个录制文件 → 播放事件数组
 * .jsonl.gz(新)透明解压;.jsonl(旧版)照常解析,向后兼容
...
function parseRecording(filePath) {
  let raw = fs.readFileSync(filePath); // Buffer
  if (/\.gz$/.test(filePath)) raw = zlib.gunzipSync(raw);
