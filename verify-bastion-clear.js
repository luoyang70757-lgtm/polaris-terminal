 ...
 * 修复后:清除历史只清 webview 浏览器会话 + 连接登录态,保留连接配置与已捕获资产。
 *
 * 断言:
 *  ① 清除前左侧渲染出 已保存连接 + H3C 捕获块 + JMS 连接
 *  ② 清除后连接配置仍在(state.settings.bastionServers / state.jmsServers 数量不变)
 *  ③ 清除后已捕获资产仍在(state.bastionAssets 不变;已保存 JMS 连接的 s.assets 不清空)
 *  ④ 清除后连接登录态被清(token=null,符合"需重新登录"提示)
 *
...
(async () => {
  ...
  try {
    ...
    if (after.includes('H3C 堡垒机')) ok('① 清除后 H3C 捕获块仍在左侧'); else bad('① 清除后 H3C 捕获块仍在左侧', after.slice(0, 200));
    ...
    await check(c, '③ 已捕获 H3C 资产未被清', `state.bastionAssets.length`, 1);
    await check(c, '③ 已保存 JMS 连接资产缓存未清', `(function(){ const s=state.jmsServers[0]; return (s.assets||[]).length; })()`, 1);
    await check(c, '④ 已保存连接登录态已清(token)', `state.settings.bastionServers[0].token`, null);
    await check(c, '④ 已保存连接 user 已清', `state.settings.bastionServers[0].user`, null);
    ...
    if (hint.includes('H3C 站点')) ok('H3C 连接展开显示捕获提示文案'); else bad('H3C 连接展开显示捕获提示文案', hint.slice(0, 120));
