...
- 架构：单文件主进程 `main.js`（~2600 行）+ 渲染进程 `src/renderer.js`（~8700 行）+ contextBridge 安全桥（`preload.js`，无 nodeIntegration）
...
5. **右键菜单防刷屏**：菜单打开后 250ms 内的 click 视为"右键残留"忽略（`ctxMenuOpenedAt`）；空菜单数组不注册 contextmenu
6. **堡垒机资产轮询**：`pollBastionAssets` 只对 H3C 站点（URL 含 `/shterm`）运行，15s 低频兜底 + 用户操作时事件驱动；数据无变化不刷新（`stableJson` 键序无关比较）
7. **全量日志**：`lib/app-log.js` 把主进程 console + 渲染层 console + dlog + 异常写入 `数据目录/logs/app-YYYYMMDD.log`（按天轮转 10MB）；调试面板「⬇ 下载日志」导出完整日志
...
已完成：堡垒机（JMS/H3C）对接、SFTP 面板全功能、菜单/焦点/刷新修复、全量日志、AI 技能/推荐/知识库。
