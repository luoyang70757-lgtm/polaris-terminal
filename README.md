...
**文件与运维**
- SFTP:浏览、上传、下载、新建/删除、远程文本编辑、全选批量操作
- SSH 隧道:本地端口转发 + 动态 SOCKS
- 批量执行(连上即跑一条命令取回输出)
- 快速命令收藏(一键发送 / 批量下发)

**录制与审计**
- 会话录制(JSONL)与回放,逐键还原操作过程
- 会话日志:每个连接落盘为可读纯文本(工具 → 打开会话日志目录),自动剥离 ANSI
- 堡垒机:内置 H3C / JumpServer Web 面板、资产捕获(会话列表直接双击资产)、`accessclient://` 免交互连接
...
- SFTP 大文件未做断点续传;录制文件按 JSONL 追加,无压缩
- 堡垒机面板为 webview 嵌入,受 Electron guest view 限制(已处理:分隔条拖动期间隐藏)
- 依赖 Electron ≥ 43(node:sqlite 内置支持)
